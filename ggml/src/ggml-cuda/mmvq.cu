...
static void mul_mat_vec_q_switch_ncols_dst(
    ...
    switch (ncols_dst) {
        ...
        // TENSELERATE: widths 9..MMVQ_MAX_BATCH_SIZE. Body is identical to cases 2..8 apart
        // from the compile-time width, so it is generated rather than copied 24 times.
        // calc_nwarps and calc_rows_per_block already have default arms for these widths
        // (nwarps 1, rows_per_block 1); they are untuned, not undefined.
#define TENSELERATE_MMVQ_CASE(N)                                                                        \
        case (N): {                                                                                     \
            constexpr int c_ncols_dst = (N);                                                            \
            std::pair<dim3, dim3> dims = calc_launch_params<type>(c_ncols_dst, nrows_x, nchannels_dst,  \
                nsamples_dst, warp_size, table_id);                                                     \
            mul_mat_vec_q_switch_fusion<type, c_ncols_dst>(vx, vy, ids, fusion, dst, ncols_x,           \
                 nchannels_y_fd, stride_row_x, stride_col_y, stride_col_dst,                            \
                 channel_ratio_fd, stride_channel_x, stride_channel_y, stride_channel_dst,              \
                 sample_ratio_fd, stride_sample_x, stride_sample_y, stride_sample_dst,                  \
                 dims.first, dims.second, 0, ids_stride, stream);                                       \
        } break;
        TENSELERATE_MMVQ_CASE( 9) TENSELERATE_MMVQ_CASE(10) TENSELERATE_MMVQ_CASE(11)
        TENSELERATE_MMVQ_CASE(12) TENSELERATE_MMVQ_CASE(13) TENSELERATE_MMVQ_CASE(14)
        TENSELERATE_MMVQ_CASE(15) TENSELERATE_MMVQ_CASE(16) TENSELERATE_MMVQ_CASE(17)
        TENSELERATE_MMVQ_CASE(18) TENSELERATE_MMVQ_CASE(19) TENSELERATE_MMVQ_CASE(20)
        TENSELERATE_MMVQ_CASE(21) TENSELERATE_MMVQ_CASE(22) TENSELERATE_MMVQ_CASE(23)
        TENSELERATE_MMVQ_CASE(24) TENSELERATE_MMVQ_CASE(25) TENSELERATE_MMVQ_CASE(26)
        TENSELERATE_MMVQ_CASE(27) TENSELERATE_MMVQ_CASE(28) TENSELERATE_MMVQ_CASE(29)
        TENSELERATE_MMVQ_CASE(30) TENSELERATE_MMVQ_CASE(31) TENSELERATE_MMVQ_CASE(32)
#undef TENSELERATE_MMVQ_CASE
        default:
