...
from tenselerate.config import (
    ATTENTION_SINK_TOKENS, DEFAULT_ATTENTION_WINDOW, QWEN38_27B, TINY,
)
...
def test_sinks_default_to_four():
    ...
    assert QWEN38_27B.attention_sink_tokens == 4


def test_sinks_are_resident_alongside_the_window():
    assert QWEN38_27B.resident_kv_tokens == \
        DEFAULT_ATTENTION_WINDOW + ATTENTION_SINK_TOKENS


def test_sinks_cost_almost_nothing():
    with_sinks = QWEN38_27B.kv_bytes_for_context(10_000_000)
    without = dataclasses.replace(QWEN38_27B, attention_sink_tokens=0)
...
def test_sinks_never_cause_position_extrapolation():
    ...
    assert QWEN38_27B.needs_rope_scaling(10_000_000) is False
    ...
    edge = dataclasses.replace(
        QWEN38_27B, attention_window=QWEN38_27B.max_position_embeddings)
